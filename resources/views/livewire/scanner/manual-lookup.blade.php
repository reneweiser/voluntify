...
>
    ...
    <div class="border-b border-zinc-700 bg-zinc-800/50 px-4 py-3">
        <flux:select wire:model.live="selectedEventId" x-on:change="offlineEventId = $event.target.value" placeholder="Select an event...">
    ...
    <div x-show="isOnline" class="flex-1 p-4">
        @if ($selectedEventId)
            ...
            @endif
        @else
            <p class="text-center text-zinc-400">Select an event to search volunteers</p>
        @endif
    </div>

    {{-- Offline: Alpine.js/IndexedDB search --}}
    <div x-show="!isOnline" x-cloak class="flex-1 p-4">
        <template x-if="offlineEventId">
            <div>
                <div class="mb-4">
                    <input
